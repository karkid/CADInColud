...
export default class DXFParser extends Parser {
    ...
    __pasrseSections(scanner) {
        ...
        while (!scanner.isEOF()) {
            if (scanner.isStartOfSection()) {
                ...
                if (group.code !== 2) {
                    console.error('Unexpected code %s after 0:SECTION', debugCode(group));
                    continue;
                }
        ...
        while (!scanner.isEndOfTable()) {
            ...
            switch (group.code) {
                ...
                case 62: // color, visibility
                    ...
                    tableRecord.color = Math.abs(group.value); //getAcadColor(Math.abs(group.value));
                    break;
    ...
    __parseBlocks(scanner) {
        ...
        while (!scanner.isEndOfSection()) {
            scanner.next();
            if (scanner.isCurrentGroup(0, 'BLOCK')) {
                block.beginBlock = this.__parseBeginBlock(scanner);
            } else if (scanner.isCurrentGroup(0, 'ENDBLK')) {
    ...
    __parseEntitiesSection(scanner) {
        ...
        while (!scanner.isEndOfSection()) {
            ...
            if (group.code === 0) {
                if (group.value === 'LINE') {
                    entities.push(LINE.parse(scanner));
                }
