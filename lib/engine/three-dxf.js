...
(function (ThreeDxf) {
    ...
    ThreeDxf.Viewer = function (data, parent, width, height, font) {
        ...
        $parent.on('click', function (event) {
            ...
            var vector = new THREE.Vector3((event.pageX - $el.offset().left) / $el.innerWidth() * 2 - 1, -((event.pageY - $el.offset().top) / $el.innerHeight()) * 2 + 1, 0.5);
        ...
        function drawMtext(entity, data) {
            var color = getColor(entity, data);
            let str = entity.text;
            let cleanString = str.replace(/[\\[f|F]+.[^\\f|F]*;+]*/g, "");

            var geometry = new THREE.TextGeometry(cleanString, {
                font: font,
        ...
        function drawLine(entity, data) {
            ...
            // set material
            if (entity.lineType) {
                lineType = data.tables.LTYPE.LTYPE[entity.lineType];
            }
        ...
        function getColor(entity, data) {
            ...
            else if (data.tables && data.tables.LAYER && data.tables.LAYER.LAYER[entity.layer])
                color = data.tables.LAYER.LAYER[entity.layer].color;
        ...
        function findExtents(scene) {
            ...
            try {
                for (var _iterator = scene.children[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
