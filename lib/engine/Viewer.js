...
export default class Viewer {
     ...
     * @param {ElmentID} parentId - the parent element to which we attach the rendering canvas
     ...
     */
    constructor(data, parentId, width, height, font) {
        ...
        var controls = new THREE.OrbitControls(this._camera, parent);
    ...
    __createScene() {
        ...
        for (let i = 0; i < this._data.entities.length; i++) {
            ...
            if (entity.type === 'DIMENSION') {
                if (entity.block) {
                    var block = this._data.blocks[entity.block];
                    if (!block) {
